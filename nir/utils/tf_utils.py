...
def get_layers(config):
    ...
    # print(type(c), c, type(tf.keras.layers.LeakyReLU), type(tf.keras.layers.LeakyReLU()))
    return c(**paras)


loss_modules = [importlib.import_module(l.__name__) for l in [tf.keras.losses, mctr.losses]]
